...
public class CmdPardonTabCompleter implements SimplifiedTabCompleter {
  ...
  public CmdPardonTabCompleter(PardonablePunishmentData punishmentData) {
  ...
  private Integer firstNonMatchingArgIndex(String[] args) {
    for (int i = 0; i < args.length; i++) {
      String arg = args[i];
      if (!arg.toUpperCase().matches(CmdPardonHandler.ID_REGEX)) {
